...
public class CustomAdapter extends RecyclerView.Adapter<CustomViewHolder> {
    ...
    public void onBindViewHolder(@NonNull CustomViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        if (headlines.get(position).getSource() != null) {
            holder.text_source.setText(headlines.get(position).getSource().getName());
        } else {
            // Xử lý khi getSource() trả về null
            holder.text_source.setText("Unknown Source");
        }
        ...
        holder.imgFavorite.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                listener.OnFavoriteClicked(headlines.get(position));
            }
        });
    }
